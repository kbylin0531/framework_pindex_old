...
//                    console.log(path.substring(path.length-3));
        ...
        init: function (config, target,cover) {
            ...
            U.each(config, function (item, key) {
                if((cover === undefined) || cover ||  target.hasOwnProperty(key)){
                    (target[key] = item);
                }
            });
            return this;
                ...
                U.each(data,function (val, key) {
                    ...
                    if (target.length) {/*表单中存在这个name的输入元素*/
                        if (target.length > 1) {/* 出现了radio或者checkbox的清空 */
                            U.each(target,function (item,k) {
                                if (('radio' === item.type) && parseInt(item.value) == parseInt(val)) {
                                    item.checked = true;
                                }
                            });
                        } else {
                            // if(target[0].tagName.toUpperCase() === 'SELECT'){
                            //     target.find("option[value="+val+"]").attr("checked","checked");
                            // }else{
                                target.val(val);
                            // }
                        }
